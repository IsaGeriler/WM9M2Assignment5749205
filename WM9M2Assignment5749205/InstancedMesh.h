...
public:
	...
	static const D3D12_INPUT_LAYOUT_DESC& getAnimatedInstancedLayout() {
		static const D3D12_INPUT_ELEMENT_DESC inputLayoutAnimatedInstanced[] = {
		{ "POSITION", 0, DXGI_FORMAT_R32G32B32_FLOAT, 0, D3D12_APPEND_ALIGNED_ELEMENT, D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, 0 },
		{ "NORMAL", 0, DXGI_FORMAT_R32G32B32_FLOAT, 0, D3D12_APPEND_ALIGNED_ELEMENT, D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, 0 },
		{ "TANGENT", 0, DXGI_FORMAT_R32G32B32_FLOAT, 0, D3D12_APPEND_ALIGNED_ELEMENT, D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, 0 },
		{ "TEXCOORD", 0, DXGI_FORMAT_R32G32_FLOAT, 0, D3D12_APPEND_ALIGNED_ELEMENT, D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, 0 },
		...
		{ "WORLD", 2, DXGI_FORMAT_R32G32B32A32_FLOAT, 1, 32, D3D12_INPUT_CLASSIFICATION_PER_INSTANCE_DATA, 1 },
		{ "WORLD", 3, DXGI_FORMAT_R32G32B32A32_FLOAT, 1, 48, D3D12_INPUT_CLASSIFICATION_PER_INSTANCE_DATA, 1 }, };
		static const D3D12_INPUT_LAYOUT_DESC desc = { inputLayoutAnimatedInstanced, 10 };
		return desc;
	}
};
...
public:
	...
	void initialize(Core* core, std::vector<STATIC_VERTEX> vertices, std::vector<unsigned int> indices, std::vector<INSTANCE> instances) {
		initialize(core, &vertices[0], sizeof(STATIC_VERTEX), vertices.size(), &indices[0], indices.size(), &instances[0], sizeof(INSTANCE), instances.size());
		inputLayoutDesc = VertexLayoutCache::getStaticInstancedLayout();
	}

	void initialize(Core* core, std::vector<ANIMATED_VERTEX> vertices, std::vector<unsigned int> indices, std::vector<INSTANCE> instances) {
		initialize(core, &vertices[0], sizeof(ANIMATED_VERTEX), vertices.size(), &indices[0], indices.size(), &instances[0], sizeof(INSTANCE), instances.size());
		inputLayoutDesc = VertexLayoutCache::getAnimatedInstancedLayout();
	}
